...
def jogo_Solitario():             #Função para jogo single player
    ...
    while True:
        if ronda_Inicial:   #ronda incial, vão ser pedidas 2 cartas ao deck
            ...
            if c == "I":                                       #Sistema de prevenção de erros 
                print(f"O deck não está atualmente disponível")
                exit(1)
            cartas_Jogador.append(c)
            c = obter_Carta()    #segunda carta 
            if c == "I":                                        #Sistema de prevenção de erros
                print(f"O deck não está atualmente disponível")
                exit(1)
            cartas_Jogador.append(c)    #Adicionar a carta à lista de cartas para depois ser calculado o score
            pontuacao = score(cartas_Jogador)
            ronda_Inicial = False    #Ronda inicial acabou, prosseguir com o jogo

        if ultima_Carta != "":       #Enquanto a última jogada do jogador n
            cartas_Jogador.append(ultima_Carta)
            pontuacao = score(cartas_Jogador)

        print(f"Pontuação Atual: {pontuacao}")    #Pontuação atual
        jogada = interact_with_user1(cartas_Jogador)  #Hash do deck, historial de jogadas
...
def main(self_port, players_ports):              #Função principal, implementação do jogo ditribuído
    ...
    while len(nao_conectados) != 0:      #Ciclo while que funciona desde que ainda haja jogadores não conectados
        print(f'Esperando {len(nao_conectados)} jogadores para se conectarem à porta {nao_conectados}')
    ...
    while True:                                                #Para o decorrer do jogo vai ser usado um ciclo while True porque não se sabe ao certo quantas rondas é que o jogo vai ter 
        ...
        if conectados[jogador_Atual][1] in aJogar:            #Passar à frente jogadores que já perderam
            if conectados[jogador_Atual][1] == self_port:     #Minha vez de jogar
                ...
                if jogada == "H":                                       #Pedir uma carta
                    ...
                    if c == "I":                               #Sistema de prevenção de erros, caso não seja disponível obter carta do deck, informar os outros jogadores que não foi possível obter a carta
                        ...
                        sock.close()
                        exit(1)
                    minhas_Cartas.append(ultima_Carta)                  #Adicionar a última carta às minhas cartas
                    pontuacao = score(minhas_Cartas)                    #Obter pontuação
                    
                    print(f"Pontuação atual: {pontuacao}")
                    print(f"Cartas atuais: {minhas_Cartas}")
                    print(f"Quer tirar outra carta? (Y/N)")              #Dar a escolher ao bad player se quer pedir uma nova carta ou não
                    resposta = ""
                    while resposta not in "YN":
                        resposta = input("> ").upper()
                    
                    if resposta == "Y":                                 #Caso a resposta seja sim, adicionar nova carta ao deck
                        ultima_Carta = obter_Carta()
                        if c == "I":                               #Sistema de prevenção de erros, caso não seja disponível obter carta do deck, informar os outros jogadores que não foi possível obter a carta
                            print(f"O deck não está atualmente disponível")
                            informa_Jogadores("I", conectados)
                            ...
                            sock.close()
                            exit(1)
                    minhas_Cartas.pop(-1)                               #Retirar a carta que tinha sido recebida originalmente, de forma a enganar o sistema
                    pontuacao = score(minhas_Cartas)
                    informa_Jogadores("H", conectados)                  #"H" == Hit/Pedir carta
                    print("Esperando pelo resto dos jogadores fazerem a sua jogada")
                elif jogada == "S" :    
                    ultima_Carta = ""
            ...
            else:                                                          #Neste bloco de código, apenas os outros jogadores vão estar a jogar, sendo que eu já fui eliminado
    ...
    for port, pontuacao_final, cards in pontuacao_Jogadores:            #Consultar a lista para ver quem tem mais pontos    
        if pontuacao_final == 21:                                       #Caso a pontuação seja igual a 21 ganha automaticamente o jogo
            ...
            vencedor = port
        elif (pontuacao_final > 21):                                    #Caso exceda os 21 pontos perde automaticamente
            break
    ...
    for jogador, j in jogadas:                                                 #Vamos percorrer a lista jogadas visto conter todas as jogadas de cada jogador
        ...
        if str(hash_deck).strip() == str(hash_jogado).strip():                   #Comparar o hash das cartas jogadas com o hash do deck
            ...
            print("Foste escolhido para verificar o hash das cartas e houve batota, queres mentir sobre o hash obtido? (S/N)") #Caso o bad_player seja escolhido para verificar o hash das cartas este pode escolher sobre mentir sobre o hash obtido ou não
            resposta = ""
            while resposta not in "YN":
                resposta = input("> ").upper()
            
            if resposta == "Y":                                                  #Caso o bad_player queira mentir sobre o hash obtido vai informar os outros jogadores que está tudo bem com o hash e está igual
                informa_Jogadores("TB", conectados[0:-2])
                ...
                informa_Jogadores("B", conectados[0:-2]) #Caso o bad_player não queira mentir sobre a comparação entre os hash, avisa os restantes a dizer que houve batotice
        ...
        while True:
            ...
            for key, mask in events:
                callback = key.data
                hash_jogado2 = callback(key.fileobj, mask)    #receive ok
                break
            break

    #Validação

    if (self_port == conectados[-2][1]) or (self_port == conectados[-1][1]):
        ...
        if hash_jogado1 == "TB" and hash_jogado2 == "TB":
            print('Não houve batota')
        else:
            print('Houve batota')
    #close
